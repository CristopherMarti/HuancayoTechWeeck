...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float lineMask = max(primaryMask, altMask);
    ...
    float cycle = dur + del;
    float scanPhase = clamp(uScrollPhase, 0.0, 1.0);
...
export const GridScan = ({
  ...
  className,
  style,
  customScrollOffset = 0, // kept for backwards compat
  scrollProgress = null // pass MotionValue here
}) => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let currentScroll = customScrollOffset;
      if (scrollProgress && typeof scrollProgress.get === 'function') {
        currentScroll = scrollProgress.get();
      }

      material.uniforms.iTime.value = now / 1000;
      material.uniforms.uScrollPhase.value = currentScroll;
